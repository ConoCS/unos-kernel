...
uint32_t subdirectory_cluster[100];
...
void ReadDirectoryFAT32(HBA_PORT *port, uint32_t subdirectory_cluster) {
    ...
    uint64_t subdirectory_sector = first_data_sector + (subdirectory_cluster - 2) * sector_per_cluster;

    ok = ahci_read(port, subdirectory_sector, sector_per_cluster, (void*)phys);
    if (!ok) {
        serial_printf("[Fatal Error] Can't read root sector\n");
    }
